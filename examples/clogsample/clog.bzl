...
def clog(name, src, sidecarOutput, **kwargs):
  native.genrule(
    ...
    cmd =  "./$(location clog_build.sh) $(location //src/clog:clogP) $(location //examples:clog_examples.clog_config) $(location //examples:clog.sidecar) $(location :%s) $(@D) %s" % (src, sidecarOutput),
